...
function getExtensionEnabled() {
  browser.runtime.sendMessage("get-extension-enabled", function (response) {
...
async function sendMessageSetExtensionEnabled(value) {
  // send to background
  browser.runtime.sendMessage({ extensionEnabled: value });
  ...
  tabs.forEach((tab) => {
    browser.tabs.sendMessage(tab.id, { extensionEnabled: value });
  });
}
...
async function sendResetDividerMessageToContentScript() {
  ...
  let tabs = await browser.tabs.query(query);
  tabs.forEach((tab) => {
    browser.tabs.sendMessage(tab.id, { message: "reset-divider" });
  });
}
